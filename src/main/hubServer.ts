...
export function startHubServer() {
          ...
          <div class="hub">
            ...
            <div class="grid">
              ...
              <div>
                ...
                <label>Clock Size</label><input type="range" min="100" max="800" value="${state.settings.bigClockSize}" oninput="upd('bigClockSize',parseInt(this.value))">
                <label>Glass Transparency</label><input type="range" min="0" max="1" step="0.01" value="${state.settings.opacity}" oninput="upd('opacity',parseFloat(this.value))">
                <br><br>
                <label>Weather City</label><input type="text" value="${state.settings.weatherCity || 'San Jose'}" onchange="upd('weatherCity', this.value)" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#fff;padding:8px;border-radius:8px;width:100%;margin:4px 0 10px 0;font-size:13px;">
                <label>Run on Startup</label> <input type="checkbox" ${state.settings.runOnStartup ? 'checked' : ''} onchange="upd('runOnStartup', this.checked)">
  ...
  server.post('/api/save-all', (req, res) => { state.saveSettings(); state.saveNotes(); state.savePositions(); res.sendStatus(200); });
  server.post('/api/settings', (req, res) => { state.updateSettings(req.body); widgetManager.updateAllStyles(); res.sendStatus(200); });
  server.post('/api/toggle/:id', (req, res) => {
    const id = req.params.id;
    state.toggleWidget(id);
    if (widgetManager.widgets.has(id)) { widgetManager.close(id); } 
    else { widgetManager.create(id.split('-')[0], id); }
    res.sendStatus(200);
  });
  server.post('/api/startup/:enabled', (req, res) => { 
    const enabled = req.params.enabled === 'true';
    state.updateSettings({ runOnStartup: enabled });
    
    const isPackaged = app.isPackaged;
    const appPath = isPackaged ? app.getPath('exe') : process.execPath;
    const args = isPackaged ? [] : [path.join(app.getAppPath(), '.')];
  ...
  server.post('/api/kill', (req, res) => { res.sendStatus(200); setTimeout(() => app.quit(), 100); });
